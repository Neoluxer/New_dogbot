...
class Dog():
    ...
    def latinica(self):
        ...
        bin_path = f'{latin_name.lower()}.bin'
        word = self.name
        words = word.split()
        print(type(words))
        for items in words:
        ...
        if (len(words_list)) >= 2:
            ...
            bin_path = f'{(transliterate.translit(bin_path2, reversed=True))}.bin'
            return bin_path
        else:
            pass
        return bin_path
...
def Dictionary_from_directory(path):
    ...
    directory = 'E:\\PycharmProjects\\tmp\\'
    directory = path
    conn.commit()
    print(conn.encoding)
    ...
    for items in Path_list:
        n = items[0:-4]
        picture_path = f'C:\\Users\\Professional\\PycharmProjects\\DogBot\\photo\\{n}\\'
...
# Dictionary_from_directory('E:\\PycharmProjects\\tmp')
